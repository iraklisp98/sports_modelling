...
- [ ] Script runs without errors: `python pipeline/stage1_ingest.py`
- [ ] Three Parquet files created: `data/processed/ENG.parquet`, `FRA.parquet`, `SPA.parquet`
- [ ] Each file has one row per match (verify with `df.count()` vs known match counts)
- [ ] No nulls in `HomeTeam`, `AwayTeam`, `Date`, `Season`
- [ ] Data quality report printed to console and written to `data/processed/quality_report.txt`
- [ ] Date range covers 2017-08 to 2020-05 for all leagues
