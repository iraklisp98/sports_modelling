...
**Full spec:** See [PRD.md](PRD.md) — this is the source of truth for scope, architecture, and success criteria.
...
| Stage | File | Status |
|---|---|---|
| 1 — Ingest & Clean | `pipeline/stage1_ingest.py` | Not started |
...
- `data/processed/`: Merged CSVs from `merge_datasets.py` — these will be replaced by Parquet in Stage 1
...
- `merge_datasets.py`: One-off merge script — will be superseded by Stage 1
- `PRD.md`: Full project spec
- `CLAUDE.md`: This file
- `LICENSE`: MIT

Nothing in `pipeline/`, `dashboard/`, `docker/`, `config/`, or `tests/` exists yet.
...
2. Read `PRD.md` for the full spec
